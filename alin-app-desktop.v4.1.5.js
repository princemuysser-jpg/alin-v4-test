...
(function(){
  ...
  function orderRow(row,role){const n=row.order_number||row.tracking_code||row.id||'—';return `<article class="alin-receipt-row"><div><b>${esc(n)} — ${esc(row.title||row.item_name||'طلب')}</b><small>${esc(date(row.created_at||row.updated_at))} • ${esc(label(row.status))}</small></div><div class="alin-receipt-row__meta"><span>${money(row.total)} د.ع</span><button type="button" onclick="AlinReceipts.openOrder('${orderKey(row)}','${esc(role)}')">معاينة وطباعة</button></div></article>`}
  function settlementRow(row,role){const n=row.receipt_number||row.voucher_number||row.id||'تسوية';return `<article class="alin-receipt-row settlement"><div><b>${esc(n)}</b><small>${esc(date(row.created_at||row.updated_at))} • ${esc(row.payment_method||row.status||'تسوية حساب')}</small></div><div class="alin-receipt-row__meta"><span>${money(row.amount)} د.ع</span><button type="button" onclick="AlinReceipts.openSettlement('${settlementKey(row)}','${esc(role)}')">معاينة وطباعة</button></div></article>`}
  function centerHtml(role){const os=[...orders(role)].sort((a,b)=>String(b.created_at||'').localeCompare(String(a.created_at||''))),ss=[...settlements(role)].sort((a,b)=>String(b.created_at||'').localeCompare(String(a.created_at||'')));const completed=os.filter(done),total=completed.reduce((s,r)=>s+Number(r.total||0),0);return `<section class="alin-receipt-center" data-receipt-role="${esc(role)}"><header class="alin-receipt-center__head"><div><small>منصة آلين</small><h2>الوصولات</h2><p>وصولات الطلبات والتسويات الخاصة بهذا الحساب فقط.</p></div><span>${os.length+ss.length}</span></header><section class="alin-receipt-metrics"><article><small>كل الطلبات</small><strong>${os.length}</strong></article><article><small>الطلبات المكتملة</small><strong>${completed.length}</strong></article><article><small>قيمة المكتمل</small><strong>${money(total)} د.ع</strong></article><article><small>التسويات</small><strong>${ss.length}</strong></article></section><div class="alin-receipt-grid"><section class="alin-receipt-panel"><div class="alin-receipt-panel__title"><h3>وصولات الطلبات</h3><span>${os.length}</span></div><div class="alin-receipt-list">${os.map(r=>orderRow(r,role)).join('')||'<div class="empty">لا توجد طلبات لهذا الحساب.</div>'}</div></section><section class="alin-receipt-panel"><div class="alin-receipt-panel__title"><h3>وصولات التسويات</h3><span>${ss.length}</span></div><div class="alin-receipt-list">${ss.map(r=>settlementRow(r,role)).join('')||'<div class="empty">لا توجد تسويات مثبتة بعد.</div>'}</div></section></div></section>`}
  function renderCenter(role,host){const target=host||document.getElementById(role==='admin'?'adminContent':role==='teacher'?'teacherContent':role==='library'?'libraryV116Content':'courierV161Content');if(!target)return false;target.innerHTML=centerHtml(role);return true}
  ...
  function openOrder(key,role=String(current()?.role||'student')){const row=scopedOrder(key,role);if(!row){window.toast?.('هذا الوصل غير متاح لهذا الحساب');return false}shell(orderReceipt(row,role),type(role,row));return true}
  function openSettlement(key,role=String(current()?.role||'')){const row=scopedSettlement(key,role);if(!row){window.toast?.('هذا الوصل غير متاح لهذا الحساب');return false}shell(settlementReceipt(row,role),'إيصال تسوية حساب');return true}
  ...
  const api=Object.freeze({renderCenter,openCenter,orders,settlements,openOrder,openSettlement,close,orderKey,settlementKey});window.AlinReceipts=api;
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',install,{once:true});else install();
})();
...
(function(){
  ...
  function shell(title,subtitle,body){close();returnFocus=document.activeElement;const root=document.createElement('div');root.id='alinDesktopDialog';root.className='alin-desktop-dialog';root.innerHTML=`<button type="button" class="alin-desktop-dialog__backdrop" aria-label="إغلاق"></button><section class="alin-desktop-dialog__panel" role="dialog" aria-modal="true" aria-label="${esc(title)}"><header><div><h2>${esc(title)}</h2><p>${esc(subtitle||'')}</p></div><button type="button" class="alin-desktop-dialog__close" aria-label="إغلاق">×</button></header><div class="alin-desktop-dialog__body">${body}</div></section>`;document.body.appendChild(root);document.body.classList.add('alin-desktop-dialog-open');root.querySelector('.alin-desktop-dialog__backdrop')?.addEventListener('click',close);root.querySelector('.alin-desktop-dialog__close')?.addEventListener('click',close);return root}
  function openTracking(initial=''){const root=shell('تتبع الطلب','اكتب رقم الطلب لمعرفة حالته الحالية.',`<section class="alin-desktop-tracking"><label for="trackOrderInput">رقم الطلب</label><div class="track-form"><input id="trackOrderInput" dir="ltr" value="${esc(initial)}" placeholder="مثال: ALIN-0001"><button type="button" data-track-submit>تتبع الآن</button></div><div class="track-result" id="trackOrderResult" aria-live="polite"></div></section>`);const input=root.querySelector('#trackOrderInput'),submit=()=>typeof window.trackOrder==='function'?window.trackOrder():null;root.querySelector('[data-track-submit]')?.addEventListener('click',submit);input?.addEventListener('keydown',event=>{if(event.key==='Enter'){event.preventDefault();submit()}});setTimeout(()=>{input?.focus();if(initial)submit()},0)}
