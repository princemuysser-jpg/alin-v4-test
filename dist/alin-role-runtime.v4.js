...
(function(){
  ...
  function filtered(){
    ...
    return orders().filter(o=>{
      const d=orderDate(o),hay=[o.order_number,o.id,o.title,o.student_name,o.student_phone,libraryName(o.library_id||o.pickup_library_id),courierName(o.courier_id||o.delegate_id),o.delivery_area,o.delivery_landmark,o.product_variant_code,o.product_variant_name].join(' ').toLowerCase();
...
(function(){
  ...
  const state={days:30,search:'',rows:[],stats:{registered:0,inactive:0},loading:false};
  ...
  function card(row){return `<article class="alin-retention-card"><div class="alin-retention-person"><div class="alin-retention-avatar">${esc((row.name||'ط').trim().slice(0,1))}</div><div><h3>${esc(row.name||'طالب')}</h3><p>${esc(row.phone||'')}</p><small>${row.grade?`المرحلة: ${esc(row.grade)} • `:''}الطلبات: ${Number(row.order_count||0)}</small></div></div><div class="alin-retention-meta"><span class="alin-retention-days">${Number(row.days_inactive||0)} يوم بدون نشاط</span><small>آخر نشاط: ${esc(fmtDate(row.last_active_at||row.last_login_at||row.created_at))}</small>${offerLabel(row)}</div><div class="alin-retention-actions"><button type="button" data-alin-click="AlinRetentionAdmin.offer" data-alin-click-arg0="${esc(row.id)}" data-alin-click-arg1="${encodeURIComponent(String(row.name||'طالب'))}">${row.active_offer_code?'إرسال عرض جديد':'إرسال عرض خاص'}</button></div></article>`}
  function render(host=document.getElementById('adminContent')){
    if(!host)return;
    host.innerHTML=`<section class="alin-retention-admin"><header class="alin-retention-head"><div><small>استرجاع العملاء</small><h2>العملاء غير النشطين</h2><p>يعتمد على آخر نشاط للحساب المسجل، وليس مجرد آخر تسجيل دخول.</p></div><button type="button" class="secondary" data-alin-click="AlinRetentionAdmin.reload">تحديث</button></header><section class="alin-retention-stats"><article><small>الحسابات المسجلة</small><b>${Number(state.stats.registered||0)}</b></article><article><small>غير النشطين حسب الفلتر</small><b>${Number(state.stats.inactive||0)}</b></article><article><small>الفترة الحالية</small><b>${state.days} يوم</b></article></section><div class="alin-retention-tools"><select id="alinRetentionDays" data-alin-change="AlinRetentionAdmin.changeDays" data-alin-change-arg0-source="value"><option value="7" ${state.days===7?'selected':''}>7 أيام</option><option value="15" ${state.days===15?'selected':''}>15 يوم</option><option value="30" ${state.days===30?'selected':''}>30 يوم</option><option value="60" ${state.days===60?'selected':''}>60 يوم</option><option value="90" ${state.days===90?'selected':''}>90 يوم</option></select><input id="alinRetentionSearch" value="${esc(state.search)}" placeholder="بحث بالاسم أو رقم الهاتف"><button type="button" data-alin-click="AlinRetentionAdmin.search">بحث</button></div><div id="alinRetentionList" class="alin-retention-list">${state.loading?'<div class="empty">جاري تحميل الحسابات...</div>':state.rows.length?state.rows.map(card).join(''):'<div class="empty">لا توجد حسابات مطابقة لهذه المدة حالياً.</div>'}</div></section>`;
  }
  async function load(){state.loading=true;render();try{const data=await rpc('alin_admin_student_retention',{p_days:state.days,p_search:state.search||null});state.rows=Array.isArray(data?.rows)?data.rows:[];state.stats=data?.stats||{registered:0,inactive:0}}catch(error){window.toast?.(error.message||'تعذر تحميل العملاء');state.rows=[]}finally{state.loading=false;render()}}
  ...
  async function sendOffer(id){
    ...
    try{
      ...
      const data=await rpc('alin_admin_create_student_offer',{p_student_id:id,p_discount_type:type,p_discount_value:value,p_days_valid:days,p_applies_to:applies,p_title:null,p_message:null});
      let push=null;
      try{
  ...
  function ensureTab(){document.querySelectorAll('#adminPage .admin-tabs').forEach(tabs=>{if(tabs.querySelector('[data-admin-tab="retention"]'))return;const b=document.createElement('button');b.type='button';b.dataset.adminTab='retention';b.setAttribute('data-alin-click','adminTab');b.setAttribute('data-alin-click-arg0','retention');b.textContent='العملاء غير النشطين';const coupons=tabs.querySelector('[data-admin-tab="coupons"]');tabs.insertBefore(b,coupons||null)})}
  const api={render,load,reload:load,offer,sendOffer,closeOffer,changeDays(v){state.days=Math.max(0,Number(v)||30);load()},search(){state.search=document.getElementById('alinRetentionSearch')?.value.trim()||'';load()}};
