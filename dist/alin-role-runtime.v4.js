...
(function(){
  ...
  const roleLabel={teacher:'مدرس',library:'مكتبة',printer:'مطبعة',courier:'مندوب',accountant:'محاسب',admin:'مدير'};
  ...
  function allAccounts(){
    ...
    const printers=arr(window.db?.accounts?.all).filter(x=>String(x.role||'').toLowerCase()==='printer'&&!x.deleted_at).map(x=>({...x,role:'printer'}));
    ...
    return [...teachers,...libraries,...printers,...couriers,...accountants];
  }
  ...
  function stats(){const a=allAccounts();return {all:a.length,active:a.filter(x=>normalizedStatus(x)==='active').length,inactive:a.filter(x=>normalizedStatus(x)==='inactive').length,teachers:a.filter(x=>x.role==='teacher').length,libraries:a.filter(x=>x.role==='library').length,printers:a.filter(x=>x.role==='printer').length,couriers:a.filter(x=>x.role==='courier').length}}
  function card(x){
  ...
  window.v131SyncAccountRole=()=>{
    ...
    const title=document.querySelector('#v131AccountForm h3');if(title)title.textContent=courier?'إضافة حساب مندوب':role==='printer'?'إضافة حساب مطبعة':'إضافة حساب';
    const save=document.getElementById('v131SaveAccountButton');if(save)save.textContent=courier?'حفظ حساب المندوب':role==='printer'?'حفظ حساب المطبعة':'حفظ الحساب';
...
(function(){
  ...
  const isPickupRole=()=>['library','printer'].includes(currentRole());
  const isPrinter=()=>currentRole()==='printer';
  const partyRole=()=>isPrinter()?'printer':'library';
  const placeLabel=()=>isPrinter()?'المطبعة':'المكتبة';
  const dbx=()=>window.db||{accounts:{libraries:[],printers:[]},orders:[],ledger:[],notifications:[]};
  function getLibrary(){
    const c=currentUser(); if(!isPickupRole()) return null;
    const accounts=dbx().accounts||{};
    const pool=[...arr(accounts.libraries),...arr(accounts.printers)];
    const ids=[c.id,c.library_id,c.account_id,c.user_id].filter(Boolean);
    return pool.find(x=>ids.some(id=>eq(x.id,id)||eq(x.account_id,id)||eq(x.user_id,id)))||
      pool.find(x=>c.username&&eq(x.username,c.username))||
      pool.find(x=>c.name&&eq(x.name,c.name))||
      c;
  }
  ...
  function notifications(){
    const role=partyRole(),id=libId();
    return window.AlinNotifications?.visible?.({role,id})||
      arr(dbx().notifications).filter(n=>n.status!=='inactive'&&
        ((n.target_id||n.library_id)?eq(n.target_id||n.library_id,id):['all',role].includes(n.target_role||n.audience)));
  }
  ...
  function financeView(){
    ...
    const movements=f.rows.slice(0,30).map(x=>`<div class="library-v120-movement"><div><b>${escx(x.order_number||x.order_id)}</b><small>${escx(x.title||'طلب مكتمل')} — استلمت ${placeLabel()} ${moneyx(x.gross)} د.ع</small></div><div class="library-v120-split"><span class="profit">ربح ${placeLabel()} +${moneyx(x.libraryProfit)} د.ع</span><span class="debt">بذمة ${placeLabel()} ${moneyx(x.debt)} د.ع</span></div></div>`).join('')||'<div class="library-v116-empty">لا توجد حركات مالية بعد</div>';
    ...
    return `<section class="library-v120-finance-cards"><article><small>إجمالي المبالغ المستلمة من الطلبات</small><strong>${moneyx(f.gross)} د.ع</strong></article><article class="profit"><small>أرباح ${placeLabel()} المتراكمة</small><strong>${moneyx(f.libraryProfit)} د.ع</strong></article><article><small>أرباح هذا الشهر</small><strong>${moneyx(f.monthProfit)} د.ع</strong></article><article class="debt"><small>المبلغ بذمة ${placeLabel()}</small><strong>${moneyx(f.debtRemaining)} د.ع</strong></article><article class="settled"><small>المبالغ المسددة للمدير</small><strong>${moneyx(f.settled)} د.ع</strong></article></section><section class="library-v116-grid library-v120-grid"><div class="library-v116-panel"><h3>تفاصيل الطلبات المالية</h3><p class="library-v120-help">عند تسليم الطلب يُثبت ربح ${placeLabel()}، ويُسجل باقي المبلغ بذمتها لحين تصفية المدير.</p><div class="library-v120-movements">${movements}</div></div><aside class="library-v116-panel"><h3>التسويات مع الإدارة</h3><div class="library-v120-debt-box"><small>المطلوب تسليمه حالياً</small><strong>${moneyx(f.debtRemaining)} د.ع</strong><span>إجمالي الذمة ${moneyx(f.debtTotal)} د.ع — المسدد ${moneyx(f.settled)} د.ع</span></div><div class="library-v116-list">${settlements}</div><div class="library-v116-note" style="margin-top:12px">التصفية يثبتها المدير فقط. بعد تسجيل كامل المبلغ تصبح الذمة صفراً، وتبقى أرباح ${placeLabel()} وسجل الحركات محفوظة.</div></aside></section>`
  }
  function notificationsView(){const ns=notifications(),role=partyRole(),label=placeLabel();return `<section class="library-v116-panel"><div class="library-v116-toolbar"><h3>إشعارات ${label}</h3><button data-alin-click="AlinLibraryV116.markAllRead">تحديد الكل كمقروء</button></div><div class="library-v116-list">${ns.map(n=>{const read=window.AlinNotifications?.isRead?.(n,{role,id:libId()})??Boolean(n.read_at||n.is_read);return `<article class="library-v116-notification ${read?'':'unread'}"><b>${escx(n.title||'إشعار')}</b><p>${escx(n.message||n.text||'')}</p><small>${escx(n.created_at||'')}</small></article>`}).join('')||'<div class="library-v116-empty">لا توجد إشعارات</div>'}</div></section>`}
  function settingsView(){const l=getLibrary()||{};return `<section class="library-v116-panel"><h3>إعدادات ${placeLabel()}</h3><div class="library-v116-settings"><div class="library-v116-field"><small>اسم ${placeLabel()}</small><b>${escx(l.name||'—')}</b></div><div class="library-v116-field"><small>المنطقة</small><b>${escx(l.area||'—')}</b></div><div class="library-v116-field"><small>أقرب نقطة دالة</small><b>${escx(l.landmark||'—')}</b></div><div class="library-v116-field"><small>واتساب</small><b>${escx(l.whatsapp||l.phone||'—')}</b></div><div class="library-v116-field"><small>اسم الدخول</small><b>${escx(l.username||currentUser()?.username||'—')}</b></div><div class="library-v116-field"><small>حالة ${placeLabel()}</small><b>${isOpen(l)?'مفتوحة':'مغلقة'}</b></div><div class="library-v116-settings-actions"><button data-alin-click="AlinLibraryV116.toggleOpen">${(isOpen(l)?'إغلاق ':'فتح ')+placeLabel()}</button><button class="secondary" data-alin-click="alert" data-alin-click-arg0="تغيير كلمة المرور يكون من إدارة الحسابات حالياً">تغيير كلمة المرور</button><button class="logout" data-alin-click="logout">تسجيل الخروج</button></div></div></section>`}
  function render(){if(!isPickupRole())return;updateHeader();document.querySelectorAll('.library-v116-tabs button').forEach(b=>b.classList.toggle('active',b.dataset.libraryTab===state.tab));const c=document.getElementById('libraryV116Content');if(!c)return;c.innerHTML=state.tab==='orders'?ordersView():state.tab==='finance'?financeView():state.tab==='notifications'?notificationsView():state.tab==='settings'?settingsView():home()}
  async function toggleOpen(){
    const lib=getLibrary();if(!lib)return alert(`تعذر تحديد حساب ${placeLabel()}`);
    ...
    try{
      ...
      if(error){
        ...
        if(/alin_set_library_open|function .* does not exist|schema cache/i.test(message))throw new Error(`خدمة حالة ${placeLabel()} غير مهيأة. حاول تسجيل الدخول من جديد، وإذا استمرت المشكلة راجع إعدادات النظام.`);
        throw error;
      }
      if(!data?.ok)throw new Error(`لم يؤكد الخادم تحديث حالة ${placeLabel()}`);
      const updated=data.library||{};
      Object.assign(lib,updated,{is_open:open,open_status:open?'open':'closed'});
      if(typeof audit==='function')await audit(partyRole(),open?`فتح ${placeLabel()}`:`إغلاق ${placeLabel()}`);
      if(typeof load==='function')await load({force:true,reason:'library-open-status'});
      render();
      if(typeof toast==='function')toast(open?`تم فتح ${placeLabel()} واستقبال طلبات الملازم`:`تم إغلاق ${placeLabel()} وإيقاف طلبات الملازم الجديدة`);
  ...
  async function markAllRead(){if(window.AlinNotifications?.markAll)await window.AlinNotifications.markAll({role:partyRole(),id:libId()});else notifications().forEach(n=>n.read_at=n.read_at||new Date().toISOString());render()}
  ...
  const boot=()=>{if(isPickupRole())setTimeout(render,20)};if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',boot);else boot();
})();
...
(function(){
  ...
  function html(s){
    return `<section class="library-v116-finance alin-printer-finance"><header class="library-v116-header" style="margin-bottom:16px"><div class="library-v116-identity"><div aria-hidden="true" class="library-v116-logo">ط</div><div><small>حساب المطبعة</small><h2>${esc(s.printer_name||window.current?.name||'المطبعة')}</h2><p>حساب الملازم وحساب توريد الكتب — كل حساب مستقل</p></div></div></header>
      <section class="library-v120-finance-cards"><article class="profit"><small>ربح المطبعة من الملازم</small><strong>${money(s.booklet_profit)} د.ع</strong></article><article><small>مبيعات الملازم</small><strong>${money(s.booklet_sales_total)} د.ع</strong></article><article><small>طلبات الملازم</small><strong>${money(s.booklet_orders_count)}</strong></article><article class="debt"><small>المتبقي من ذمة الملازم</small><strong>${money(s.collector_remaining)} د.ع</strong></article></section>
      <section class="library-v116-grid library-v120-grid" style="margin-top:16px"><div class="library-v116-panel"><h3>حساب الملازم</h3><p class="library-v120-help">خاص بطباعة وتسليم الملازم فقط، ومنفصل بالكامل عن توريد الكتب.</p><div class="library-v120-debt-box"><small>ربح المطبعة من الملازم</small><strong>${money(s.booklet_profit)} د.ع</strong><span>إجمالي مبيعات الملازم ${money(s.booklet_sales_total)} د.ع — عدد الطلبات ${money(s.booklet_orders_count)}</span></div><div class="library-v120-debt-box" style="margin-top:10px"><small>ذمة الملازم</small><strong>${money(s.collector_remaining)} د.ع</strong><span>إجمالي الذمة ${money(s.collector_debt_total)} د.ع — المسدد ${money(s.collector_settled)} د.ع</span></div></div><aside class="library-v116-panel"><h3>تسويات حساب الملازم</h3><div class="library-v116-list">${settlementRows(s.collector_settlements,'تسوية ملازم')}</div></aside></section>
...
(function(){
  ...
  function purchaseLabel(o){if(String(o?.kind||'')==='booklet')return '';return String(o?.purchase_type||'unit')==='pack'?`باكيت${Number(o?.pack_size||0)>=2?` (${Number(o.pack_size)} قطع)`:''}`:'مفرد'}
  ...
  function details(id){
    const o=orders().find(x=>String(x.id)===String(id));if(!o)return;const m=orderMeta(id),prep=bookletPrep(o),matches=homeDelivery(o)?matchingCouriers(o.delivery_area):couriers(),assigned=String(o.courier_id||o.delegate_id||'');
  ...
  window.adminOrdersExport=()=>{const rows=[['رقم الطلب','الطالب','الهاتف','العنصر','كود التصميم','اسم التصميم','نوع الشراء','الكمية','الإجمالي','الخصم','أجرة التوصيل','الحالة','المكتبة','المندوب','المنطقة','التاريخ'],...filtered().map(o=>[o.order_number||o.id,o.student_name||'',o.student_phone||'',o.title||'',o.product_variant_code||'',o.product_variant_name||'',purchaseLabel(o),o.qty||1,o.total||0,o.discount||0,o.delivery_fee||0,labelOf(o),libraryName(o.library_id||o.pickup_library_id),courierName(o.courier_id||o.delegate_id),normalizeArea(o.delivery_area),dateText(o)])];const csv='\ufeff'+rows.map(r=>r.map(v=>`"${String(v).replace(/"/g,'""')}"`).join(',')).join('\n');const blob=new Blob([csv],{type:'text/csv;charset=utf-8'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`alin-orders-${new Date().toISOString().slice(0,10)}.csv`;a.click();setTimeout(()=>URL.revokeObjectURL(a.href),250)};
})();
;
...
(function(){
  ...
  function render(content=root()){
    ...
    content.innerHTML=`
      ...
      <section class="as144-panel" data-as144-panel="profits"><div class="as144-card"><h3>إعدادات الأرباح</h3><div class="as144-note">نسب المنصة والمدرس والمكتبة لم تعد نسباً عامة. يتم تحديدها بصورة مستقلة داخل كل ملزمة، ومجموعها يجب أن يساوي 100%.</div><div class="as144-grid">
        <div class="as144-field"><label>عمولة المندوب من أجرة التوصيل %</label><input id="as144CourierProfit" type="number" min="0" max="100" value="${escv(value('delegate_profit_percent','30'))}"></div>
      </div><div class="as144-actions"><button class="as144-save" data-save="profits">حفظ عمولة المندوب</button><button class="secondary" data-alin-click="adminTab" data-alin-click-arg0="booklets">فتح الملازم وتعديل نسبها</button></div><div id="as144ProfitsMsg" class="as144-status"></div></div></section>
    ...
    content.querySelector('[data-save="profits"]')?.addEventListener('click',()=>saveMany({delegate_profit_percent:document.getElementById('as144CourierProfit').value||30},document.getElementById('as144ProfitsMsg')).catch(()=>{}));
...
(function(){
  ...
  window.AlinCourierCore=Object.freeze({
    version:window.ALIN_CONFIG?.version||'4.2.0',$, $$, arr, escv, moneyv, now, notify, currentAccount, dbx,
...
(function(){
  ...
  function renderCouriersAdmin(){
    ...
    adminContent.innerHTML=`<section class="v164-admin-couriers"><header class="v164-admin-head"><div><small>نظام التوصيل</small><h2>إدارة المندوبين</h2><p>الحسابات والمناطق والحالة والطلبات والذمم من مكان واحد.</p></div><div></div></header><section class="v164-admin-metrics"><article><small>إجمالي المندوبين</small><strong>${rows.length}</strong></article><article><small>فعالون</small><strong>${rows.filter(c=>c.status!=='inactive').length}</strong></article><article><small>متاحون</small><strong>${rows.filter(c=>statusOf(c)==='available').length}</strong></article><article><small>طلبات جارية</small><strong>${rows.reduce((sum,c)=>sum+activeLoad(c),0)}</strong></article><article><small>إجمالي الذمم</small><strong>${moneyv(debt)} د.ع</strong></article></section><section class="v164-admin-tools"><input id="v216CourierQ" placeholder="بحث بالاسم أو الهاتف أو المنطقة" data-alin-input="alinV216FilterCouriers"><select id="v216CourierStatus" data-alin-change="alinV216FilterCouriers"><option value="">كل الحالات</option><option value="available">متاح</option><option value="busy">مشغول</option><option value="offline">خارج الخدمة</option><option value="inactive">موقوف</option></select><select id="v216CourierArea" data-alin-change="alinV216FilterCouriers"><option value="">كل المناطق</option>${areas.map(a=>`<option value="${escv(a)}">${escv(a)}</option>`).join('')}</select></section><div class="v164-admin-grid" id="v216CourierGrid">${rows.map(adminCourierCard).join('')||'<div class="empty">لا يوجد مندوبون بعد.</div>'}</div></section>`;
  }
...
(function(){
  ...
  const VERSION='4.2.0';
  const SOUND_URL='./assets/audio/alin-order-chime.wav?v=4.2.0';
...
(function(){
  ...
  function bind(root){
    ...
    const apply=()=>{
      ...
      let shown=0;
      root.dataset.alin415rKindFilter=kind;
      rows().forEach(row=>{
        ...
        if(visible){row.style.removeProperty('display');shown++;}
    ...
    root.addEventListener('click',event=>{
      const filterButton=event.target.closest('[data-alin415r-filter]');
      if(filterButton){
        event.preventDefault();event.stopPropagation();
        kind=filterButton.dataset.alin415rFilter||'all';
        root.querySelectorAll('[data-alin415r-filter]').forEach(item=>{
