...
(function(){
  ...
  const statusName=status=>({published:'منشورة',active:'منشورة',hidden:'مخفية',draft:'مسودة',pending:'قيد المراجعة',under_review:'قيد المراجعة',review:'قيد المراجعة',rejected:'مرفوضة',approved:'منشورة'}[String(status||'').toLowerCase()]||status||'غير محددة');
...
(function(){
  ...
  const statusMap={new:'قيد الاستلام',pending:'قيد المراجعة',designing:'قيد التصميم',under_review:'قيد المراجعة',review:'قيد المراجعة',ready:'جاهزة للموافقة',approved:'تمت الموافقة',published:'منشورة',rejected:'مرفوضة'};
  const statusName=s=>statusMap[String(s||'new').toLowerCase()]||s||'قيد المراجعة';
  const statusStep=s=>{s=String(s||'new').toLowerCase();if(['rejected'].includes(s))return 1;if(['new','pending'].includes(s))return 1;if(['designing'].includes(s))return 2;if(['under_review','review','ready'].includes(s))return 3;if(['approved','published'].includes(s))return 4;return 1};
...
(function(){
  ...
  const statusLabels={new:'تم الإرسال',pending:'بانتظار المراجعة',under_review:'قيد المراجعة',review:'قيد المراجعة',changes_requested:'مطلوب تعديل',resubmitted:'أعيد الإرسال',approved:'تمت الموافقة',ready:'جاهزة للنشر',published:'منشورة',rejected:'مرفوضة',designing:'قيد التجهيز'};
  const label=s=>statusLabels[String(s||'new').toLowerCase()]||String(s||'قيد المراجعة');
  const step=s=>{s=String(s||'new').toLowerCase();if(s==='rejected')return 2;if(['new','pending'].includes(s))return 1;if(['under_review','review','designing'].includes(s))return 2;if(['changes_requested'].includes(s))return 3;if(['resubmitted','approved','ready'].includes(s))return 4;if(s==='published')return 5;return 1};
  ...
  window.alinV152AdminDecision=async function(id,status){if(status==='review')status='under_review';const note=document.getElementById('v152AdminNote-'+id)?.value.trim()||'';if(['changes_requested','rejected'].includes(status)&&!note)return alert('اكتب ملاحظة أو سبب واضح للمدرس');try{await update('teacher_requests',{status,admin_note:note,reviewed_at:new Date().toISOString(),reviewed_by:cur().name||cur().username||'admin',updated_at:new Date().toISOString()},{id});if(typeof audit==='function')await audit('teacher_request',`تحديث طلب ${id} إلى ${status}`);if(typeof load==='function')await load();renderAdminReview();if(typeof toast==='function')toast('تم تحديث حالة الطلب')}catch(e){console.warn(e);alert(e.message||'تعذر تحديث الطلب')}};
  if(!window.TeacherApp)throw new Error('TeacherApp must load before teacher/publishing.js');
...
(function(){
  ...
  const statusLabel=status=>({published:'منشورة',hidden:'مخفية',draft:'مسودة',under_review:'قيد المراجعة',review:'قيد المراجعة',pending:'قيد المراجعة',archived:'مؤرشفة'}[String(status||'draft').toLowerCase()]||String(status||'مسودة'));
