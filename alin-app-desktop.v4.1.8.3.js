...
(function(){
  ...
  function renderDashboard(context){
    ...
    const orderHtml=recent.map(order=>`<div class="teacher-v148-order"><div><b>${escSafe(order.order_number||order.id)} — ${escSafe(order.title||'طلب ملزمة')}</b><small>${escSafe(statusName(order.status))} • ${+order.qty||1} نسخة • ${dateOnly(order.created_at)||'-'}</small></div><div class="alin-receipt-inline"><strong>${moneySafe(order.total)} د.ع</strong><button type="button" class="alin-receipt-command" onclick="AlinReceipts.printOrder('${escSafe(order.id)}')">طباعة الوصل</button></div></div>`).join('')||'<div class="teacher-v148-empty">لا توجد طلبات حديثة.</div>';
...
(function(){
  ...
  function financeView(){
    ...
    const settlements=f.settlements.slice(0,15).map(x=>`<div class="library-v116-row"><div><b>${escx(x.receipt_number||x.id||'تسوية')}</b><small>${escx(x.created_at||'')} — ${escx(x.payment_method||'')}</small></div><div class="library-v116-actions"><span class="library-v116-money settled">-${moneyx(x.amount)} د.ع</span><button class="secondary" onclick="AlinReceipts.printSettlement('library','${escx(x.id||x.receipt_number)}')">طباعة وصل التسوية</button></div></div>`).join('')||'<div class="library-v116-empty">لا توجد تسويات مثبتة بعد</div>';
...
(function(){
  ...
  function settlementCards(id){
    const rows=summary(id).settlements.slice().sort((a,b)=>String(b.created_at||'').localeCompare(String(a.created_at||'')));
    return rows.map(row=>`<div class="library-v116-row"><div><b>${escv(row.receipt_number||row.id)}</b><small>${escv(String(row.created_at||'').slice(0,10))} • ${escv(row.payment_method||'نقدي')}</small></div><span>${moneyv(row.amount)} د.ع</span><button class="secondary" onclick="AlinReceipts.printSettlement('library','${escv(row.id||row.receipt_number)}')">طباعة الوصل</button></div>`).join('')||'<div class="library-v116-empty">لا توجد سندات تسوية سابقة.</div>';
  }
...
(function(){
  ...
  function withdrawals(){
    ...
    return rows.map(row=>`<div class="admin-v137-finance-row"><div><b>${escv(row.role||'حساب')} — ${moneyv(row.amount)} د.ع</b><small>${escv(row.account_id||'')} • ${escv(String(row.created_at||'').slice(0,10))}</small></div><div class="row-actions"><span>${escv(row.status||'pending')}</span><button class="secondary" onclick="AlinReceipts.printWithdrawal('${escv(row.id)}')">طباعة وصل السحب</button>${row.status==='pending'?`<button onclick="withdrawStatus('${escv(row.id)}','approved')">موافقة</button><button onclick="withdrawStatus('${escv(row.id)}','paid')">دفع</button><button class="danger" onclick="withdrawStatus('${escv(row.id)}','rejected')">رفض</button>`:''}</div></div>`).join('')||'<div class="empty">لا توجد طلبات سحب.</div>';
  }
  ...
  function settlementRows(){
    ...
    return rows.map(row=>`<div class="admin-v137-finance-row"><div><b>${escv(row.name||row.kind)}</b><small>${escv(row.number||'')} • ${escv(String(row.created_at||'').slice(0,10))} • ${escv(row.method||'نقدي')}</small></div><div><strong>${moneyv(row.amount)} د.ع</strong><span>${escv(row.status||'')}</span><button class="secondary" onclick="AlinReceipts.printSettlement('${escv(row.kind)}','${escv(row.id)}')">طباعة الوصل</button></div></div>`).join('')||'<div class="empty">لا توجد سندات أو تسويات.</div>';
  }
...
(function(){
  ...
  function financeHtml(c,rows){const f=financials(c),doneRows=rows.filter(done);return `${summary(c,rows)}<section class="v164-finance-grid"><article><small>المبالغ المستلمة</small><strong>${moneyv(f.collected)} د.ع</strong></article><article><small>أرباح التوصيل</small><strong>${moneyv(f.earnings)} د.ع</strong></article><article><small>المسدّد للإدارة</small><strong>${moneyv(f.paid)} د.ع</strong></article><article class="debt"><small>المبلغ بذمتك</small><strong>${moneyv(f.debt)} د.ع</strong></article></section><section class="v164-table-card"><h2>كشف الطلبات المالية</h2><div class="v164-finance-list">${doneRows.map(o=>`<div><span>${escv(o.order_number||o.id)}</span><span>${moneyv(o.total)} د.ع</span><span>ربح التوصيل ${moneyv(o.delegate_profit||o.courier_profit||window.AlinFinance?.shares?.(o)?.delegate||0)} د.ع</span><span>${escv(fmtDate(o.delivered_at||o.updated_at))}</span><button type="button" class="alin-receipt-command" onclick="AlinReceipts.printDelivery('${escv(o.id)}')">طباعة وصل التسليم</button></div>`).join('')||'<p class="empty">لا توجد حركات مالية بعد.</p>'}</div></section>`}
...
(function(){
  ...
  window.addEventListener('load',async()=>{
    try{
      const registration=await navigator.serviceWorker.register('./service-worker.js?v=4.1.8.1',{scope:'./',updateViaCache:'none'});
