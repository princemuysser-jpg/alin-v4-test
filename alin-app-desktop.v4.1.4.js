...
// === store/tracking.js ===
/* ALIN v4.1.4 — public order tracking compatible with table and JSON RPC responses. */
(function(){
  'use strict';
  ...
  window.trackOrder=async function(){
    ...
    try{
      ...
      const {data,error}=await client.rpc('alin_track_order',{p_order_number:code});
      if(error)throw error;
      const row=rowFromRpc(data);
      if(!row){box.textContent='لم يتم العثور على الطلب. تأكد من رقم التتبع.';return false}
      renderTracking(box,row,code);
      return true;
    }catch(error){
      console.error('[ALIN tracking v4.1.4]',error);
      const message=String(error?.message||'');
