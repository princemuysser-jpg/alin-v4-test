...
/* ALIN v4.2.0 — unified, role-scoped receipt center. */
(function(){
  'use strict';

  const STATUS_LABELS={
    pending:'قيد الانتظار',new:'جديد',pending_admin:'بانتظار الإدارة',payment_pending:'بانتظار التأكيد',
    processing:'قيد التجهيز',printing:'قيد الطباعة',accepted:'مقبول',ready:'جاهز',assigned:'مُسند',
    picked_up:'تم الاستلام',out_for_delivery:'قيد التوصيل',out_delivery:'قيد التوصيل',
    completed:'تم التسليم',delivered:'تم التسليم',cancelled:'ملغي',canceled:'ملغي',rejected:'مرفوض'
  };
  ...
  const money=value=>Number(value||0).toLocaleString(window.AlinI18n?.locale?.()||'ar-IQ',{maximumFractionDigits:0});
  const formatDate=value=>{
    const date=value?new Date(value):new Date();
    return Number.isNaN(date.getTime())?'—':date.toLocaleString(window.AlinI18n?.locale?.()||'ar-IQ',{dateStyle:'medium',timeStyle:'short'});
  };
  const statusLabel=value=>STATUS_LABELS[String(value||'').toLowerCase()]||String(value||'—');
  const current=()=>window.current||null;
  const accountId=()=>String(current()?.id||current()?.account_id||current()?.user_id||'');
  const db=()=>window.db||{};

  function teacherBookIds(id){
    return new Set(arr(db().booklets).filter(row=>same(row.teacher_id,id)).map(row=>String(row.id)));
  }
  function roleOrders(role=String(current()?.role||'')){
    const id=accountId();
    const rows=arr(db().orders);
    if(role==='admin'||role==='accountant')return rows;
    if(role==='teacher'){
      const books=teacherBookIds(id);
      return rows.filter(row=>same(row.teacher_id,id)||books.has(String(row.item_id||row.booklet_id||'')));
    }
    ...
    if(role==='student')return arr(window.AlinStudentAuth?.orders?.());
    return [];
  }
  function roleSettlements(role=String(current()?.role||'')){
    const id=accountId();
    if(role==='admin'||role==='accountant'){
      return [
        ...arr(db().librarySettlements||db().library_settlements),
        ...arr(db().teacherSettlements||db().teacher_settlements),
        ...arr(db().courierSettlements||db().delegate_settlements),
        ...arr(db().adminSettlements||db().admin_settlements)
      ];
    }
    if(role==='teacher')return arr(db().teacherSettlements||db().teacher_settlements).filter(row=>same(row.teacher_id||row.party_id,id));
    if(role==='library')return arr(db().librarySettlements||db().library_settlements).filter(row=>same(row.library_id||row.party_id,id));
    if(role==='courier')return arr(db().courierSettlements||db().delegate_settlements).filter(row=>same(row.courier_id||row.delegate_id||row.party_id,id));
    return [];
  }
  function scopedOrder(key,role=String(current()?.role||'')){
    const decoded=decodeURIComponent(String(key||''));
    return roleOrders(role).find(row=>[row.id,row.order_id,row.order_number,row.tracking_code].some(value=>same(value,decoded)))||null;
  }
  function scopedSettlement(key,role=String(current()?.role||'')){
    const decoded=decodeURIComponent(String(key||''));
    return roleSettlements(role).find(row=>[row.id,row.receipt_number,row.voucher_number].some(value=>same(value,decoded)))||null;
  }
  function partyName(role,row={}){
    if(role==='teacher')return current()?.name||'المدرس';
    if(role==='library')return current()?.name||'المكتبة';
    if(role==='courier')return current()?.name||'المندوب';
    if(role==='student')return window.AlinStudentAuth?.current?.()?.name||'الطالب';
    return row.student_name||'إدارة منصة آلين';
  }
  ...
  function isDone(row){return ['completed','delivered'].includes(String(row?.status||'').toLowerCase());}
  function receiptType(role,row){
    if(role==='courier')return isDone(row)?'إيصال تسليم مندوب':'إيصال طلب توصيل';
    if(role==='library')return isDone(row)?'إيصال تسليم طلب':'إيصال استلام طلب';
    if(role==='teacher')return 'إيصال بيع ملزمة';
    if(role==='student')return 'إيصال طلب الطالب';
    return isDone(row)?'إيصال طلب مكتمل':'إيصال طلب';
  }
  function orderCard(row,role){
    const number=row.order_number||row.tracking_code||row.id||row.order_id||'—';
    return `<article class="alin-receipt-row"><div><b>${esc(number)} — ${esc(row.title||row.item_name||'طلب')}</b><small>${esc(formatDate(row.created_at||row.updated_at))} • ${esc(statusLabel(row.status))}</small></div><div class="alin-receipt-row__meta"><span>${money(row.total)} د.ع</span><button type="button" onclick="AlinReceipts.openOrder('${orderKey(row)}','${esc(role)}')">معاينة وطباعة</button></div></article>`;
  }
  function settlementCard(row,role){
    const number=row.receipt_number||row.voucher_number||row.id||'تسوية';
    return `<article class="alin-receipt-row settlement"><div><b>${esc(number)}</b><small>${esc(formatDate(row.created_at||row.updated_at))} • ${esc(row.payment_method||row.status||'تسوية حساب')}</small></div><div class="alin-receipt-row__meta"><span>${money(row.amount)} د.ع</span><button type="button" onclick="AlinReceipts.openSettlement('${settlementKey(row)}','${esc(role)}')">معاينة وطباعة</button></div></article>`;
  }
  function centerHtml(role){
    const orders=[...roleOrders(role)].sort((a,b)=>String(b.created_at||'').localeCompare(String(a.created_at||'')));
    const settlements=[...roleSettlements(role)].sort((a,b)=>String(b.created_at||'').localeCompare(String(a.created_at||'')));
    const completed=orders.filter(isDone).length;
    const total=orders.filter(isDone).reduce((sum,row)=>sum+Number(row.total||0),0);
    return `<section class="alin-receipt-center" data-receipt-role="${esc(role)}"><header class="alin-receipt-center__head"><div><small>منصة آلين</small><h2>الوصولات</h2><p>وصولات الطلبات والتسويات الخاصة بهذا الحساب فقط.</p></div><span>${orders.length+settlements.length}</span></header><section class="alin-receipt-metrics"><article><small>كل الطلبات</small><strong>${orders.length}</strong></article><article><small>الطلبات المكتملة</small><strong>${completed}</strong></article><article><small>قيمة المكتمل</small><strong>${money(total)} د.ع</strong></article><article><small>التسويات</small><strong>${settlements.length}</strong></article></section><div class="alin-receipt-grid"><section class="alin-receipt-panel"><div class="alin-receipt-panel__title"><h3>وصولات الطلبات</h3><span>${orders.length}</span></div><div class="alin-receipt-list">${orders.map(row=>orderCard(row,role)).join('')||'<div class="empty">لا توجد طلبات لهذا الحساب.</div>'}</div></section><section class="alin-receipt-panel"><div class="alin-receipt-panel__title"><h3>وصولات التسويات</h3><span>${settlements.length}</span></div><div class="alin-receipt-list">${settlements.map(row=>settlementCard(row,role)).join('')||'<div class="empty">لا توجد تسويات مثبتة بعد.</div>'}</div></section></div></section>`;
  }
  function renderCenter(role,host){
    const target=host||document.getElementById(role==='admin'?'adminContent':role==='teacher'?'teacherContent':role==='library'?'libraryV116Content':'courierV161Content');
    if(!target)return false;
    target.innerHTML=centerHtml(role);
    return true;
  }
  function receiptShell(content,title){
    document.getElementById('alinReceiptDialog')?.remove();
    const overlay=document.createElement('div');
    overlay.id='alinReceiptDialog';overlay.className='alin-receipt-dialog';
    overlay.innerHTML=`<button class="alin-receipt-backdrop" type="button" aria-label="إغلاق"></button><section class="alin-receipt-dialog__panel" role="dialog" aria-modal="true" aria-label="${esc(title)}"><div class="alin-receipt-toolbar no-print"><button type="button" data-receipt-print>طباعة / حفظ PDF</button><button type="button" class="secondary" data-receipt-close>إغلاق</button></div>${content}</section>`;
    document.body.appendChild(overlay);document.body.classList.add('alin-receipt-open');
    const close=()=>{overlay.remove();document.body.classList.remove('alin-receipt-open')};
    overlay.querySelector('[data-receipt-close]')?.addEventListener('click',close);
    overlay.querySelector('.alin-receipt-backdrop')?.addEventListener('click',close);
    overlay.querySelector('[data-receipt-print]')?.addEventListener('click',()=>window.print());
    return overlay;
  }
  function orderReceiptHtml(row,role){
    const number=row.order_number||row.tracking_code||row.id||row.order_id||'—';
    const qty=Math.max(1,Number(row.qty||1));
    const unit=Number(row.unit_price||row.price||((Number(row.total||0)-Number(row.delivery_fee||0))/qty)||0);
    const delivery=Number(row.delivery_fee||0);
    const total=Number(row.total||0);
    const customer=row.student_name||partyName(role,row);
    const phone=row.student_phone||'';
    const location=row.fulfillment_type==='home_delivery'||row.delivery_type==='courier'?(row.delivery_area||row.delivery_landmark||'توصيل للبيت'):(row.library_name||'استلام من المكتبة');
    return `<article class="alin-print-receipt" id="alinPrintableReceipt" dir="rtl"><header><div class="alin-print-brand"><span>آ</span><div><h1>منصة آلين</h1><p>ملازم • قرطاسية • هدايا</p></div></div><div class="alin-print-receipt__number"><small>${esc(receiptType(role,row))}</small><b>${esc(number)}</b></div></header><section class="alin-print-info"><div><small>التاريخ</small><b>${esc(formatDate(row.created_at||row.updated_at))}</b></div><div><small>الحالة</small><b>${esc(statusLabel(row.status))}</b></div><div><small>اسم الطالب</small><b>${esc(customer||'—')}</b></div><div><small>رقم الهاتف</small><b dir="ltr">${esc(phone||'—')}</b></div><div><small>طريقة الاستلام</small><b>${esc(location)}</b></div><div><small>الجهة</small><b>${esc(partyName(role,row))}</b></div></section><table><thead><tr><th>الصنف</th><th>الكمية</th><th>سعر الوحدة</th><th>الإجمالي</th></tr></thead><tbody><tr><td>${esc(row.title||row.item_name||'طلب منصة آلين')}</td><td>${qty}</td><td>${money(unit)} د.ع</td><td>${money(Math.max(0,total-delivery))} د.ع</td></tr>${delivery?`<tr><td>أجرة التوصيل</td><td>1</td><td>${money(delivery)} د.ع</td><td>${money(delivery)} د.ع</td></tr>`:''}</tbody></table><section class="alin-print-total"><span>المبلغ الكلي</span><strong>${money(total)} د.ع</strong></section><section class="alin-print-notes"><div><small>ملاحظات</small><p>${esc(row.notes||row.note||row.delivery_note||'لا توجد ملاحظات')}</p></div><div class="alin-print-sign"><span>توقيع المستلم</span><span>توقيع الجهة</span></div></section><footer>شكراً لاستخدام منصة آلين</footer></article>`;
  }
  function settlementReceiptHtml(row,role){
    const number=row.receipt_number||row.voucher_number||row.id||'—';
    const amount=Number(row.amount||0);
    return `<article class="alin-print-receipt settlement" id="alinPrintableReceipt" dir="rtl"><header><div class="alin-print-brand"><span>آ</span><div><h1>منصة آلين</h1><p>ملازم • قرطاسية • هدايا</p></div></div><div class="alin-print-receipt__number"><small>إيصال تسوية حساب</small><b>${esc(number)}</b></div></header><section class="alin-print-info"><div><small>التاريخ</small><b>${esc(formatDate(row.created_at||row.updated_at))}</b></div><div><small>الجهة</small><b>${esc(partyName(role,row))}</b></div><div><small>نوع الجهة</small><b>${esc(role==='teacher'?'مدرس':role==='library'?'مكتبة':role==='courier'?'مندوب':'إدارة')}</b></div><div><small>طريقة التسوية</small><b>${esc(row.payment_method||'نقداً')}</b></div><div><small>الحالة</small><b>${esc(row.status||'مثبتة')}</b></div><div><small>رقم الوصل</small><b>${esc(number)}</b></div></section><section class="alin-settlement-summary"><div><small>المبلغ المسدد</small><strong>${money(amount)} د.ع</strong></div><div><small>ملاحظات</small><p>${esc(row.note||row.notes||'تسوية حساب مثبتة في منصة آلين')}</p></div></section><section class="alin-print-notes"><div class="alin-print-sign"><span>توقيع منصة آلين</span><span>توقيع الجهة</span></div></section><footer>شكراً لاستخدام منصة آلين</footer></article>`;
  }
  function openOrder(key,role=String(current()?.role||'student')){
    const row=scopedOrder(key,role);
    if(!row){window.toast?.('هذا الوصل غير متاح لهذا الحساب');return false}
    receiptShell(orderReceiptHtml(row,role),receiptType(role,row));return true;
  }
  function openSettlement(key,role=String(current()?.role||'')){
    const row=scopedSettlement(key,role);
    if(!row){window.toast?.('هذا الوصل غير متاح لهذا الحساب');return false}
    receiptShell(settlementReceiptHtml(row,role),'إيصال تسوية حساب');return true;
  }
  function close(){document.getElementById('alinReceiptDialog')?.remove();document.body.classList.remove('alin-receipt-open')}
  function installRoleTabs(){
    window.TeacherApp?.registerTab?.('receipts',()=>renderCenter('teacher',document.getElementById('teacherContent')));
    window.AlinAdminModules?.register?.('receipts',host=>renderCenter('admin',host));
  }

  const api=Object.freeze({renderCenter,roleOrders,roleSettlements,openOrder,openSettlement,close,orderKey,settlementKey});
  window.AlinReceipts=api;
  installRoleTabs();
...
(function(){
  ...
  function render(){if(currentUser()?.role!=='library')return;updateHeader();document.querySelectorAll('.library-v116-tabs button').forEach(b=>b.classList.toggle('active',b.dataset.libraryTab===state.tab));const c=document.getElementById('libraryV116Content');if(!c)return;if(state.tab==='receipts'){window.AlinReceipts?.renderCenter?.('library',c);return}c.innerHTML=state.tab==='orders'?ordersView():state.tab==='finance'?financeView():state.tab==='notifications'?notificationsView():state.tab==='settings'?settingsView():home()}
...
(function(){
  ...
  async function secureCheckout(){
    ...
    try{
      ...
      if(signedStudent){
        const nameInput=document.getElementById('studentName'),phoneInput=document.getElementById('studentPhone');
        if(nameInput)nameInput.value=name;if(phoneInput)phoneInput.value=phone;
      }
...
/* modules/store/desktop-tools.js */
/* ALIN v4.2.0 — desktop options and order tracking dialogs. */
(function(){
  'use strict';
  ...
  function close(){
    document.getElementById('alinDesktopDialog')?.remove();
    document.body.classList.remove('alin-desktop-dialog-open');
    try{returnFocus?.focus?.()}catch(_){}
    returnFocus=null;
  }
  function shell(title,subtitle,body){
    close();returnFocus=document.activeElement;
    const root=document.createElement('div');root.id='alinDesktopDialog';root.className='alin-desktop-dialog';
    root.innerHTML=`<button type="button" class="alin-desktop-dialog__backdrop" aria-label="إغلاق"></button><section class="alin-desktop-dialog__panel" role="dialog" aria-modal="true" aria-label="${esc(title)}"><header><div><h2>${esc(title)}</h2><p>${esc(subtitle||'')}</p></div><button type="button" class="alin-desktop-dialog__close" aria-label="إغلاق">×</button></header><div class="alin-desktop-dialog__body">${body}</div></section>`;
    document.body.appendChild(root);document.body.classList.add('alin-desktop-dialog-open');
    root.querySelector('.alin-desktop-dialog__backdrop')?.addEventListener('click',close);
    root.querySelector('.alin-desktop-dialog__close')?.addEventListener('click',close);
    return root;
  }
  function openTracking(initial=''){
    const root=shell('تتبع الطلب','اكتب رقم الطلب لمعرفة حالته الحالية.',`<section class="alin-desktop-tracking"><label for="trackOrderInput">رقم الطلب</label><div class="track-form"><input id="trackOrderInput" dir="ltr" value="${esc(initial)}" placeholder="مثال: AL-260802-0001"><button type="button" data-track-submit>تتبع الآن</button></div><div class="track-result" id="trackOrderResult" aria-live="polite"></div></section>`);
  ...
  function optionButton(icon,title,subtitle,action){return `<button type="button" class="alin-desktop-option" data-option-action="${esc(action)}"><span>${icon}</span><div><b>${esc(title)}</b><small>${esc(subtitle)}</small></div><i>‹</i></button>`}
  function openOptions(){
    const student=window.AlinStudentAuth?.current?.();
    const theme=document.documentElement.dataset.alinTheme==='dark'?'dark':'light';
    const language=window.AlinI18n?.current?.()||'ar';
    const root=shell('الخيارات','الحساب والتتبع واللغة ومظهر المنصة.',`<section class="alin-desktop-options"><div class="alin-desktop-options__profile"><span>👤</span><div><b>${esc(student?.name||'حساب الطالب')}</b><small>${esc(student?.phone||'سجل الدخول لمشاهدة طلباتك ووصولاتك')}</small></div></div>${optionButton('👤','حسابي','صفحة الطالب والطلبات والوصولات','account')}${optionButton('⌖','تتبع الطلب','متابعة حالة أي طلب برقم التتبع','tracking')}<div class="alin-desktop-option-row"><span>🌐</span><div><b>اللغة</b><small>لغة واجهة المنصة</small></div><div class="alin-desktop-choice-group"><button type="button" data-language="ar" class="${language==='ar'?'active':''}">العربية</button><button type="button" data-language="ku" class="${language==='ku'?'active':''}">کوردی</button><button type="button" data-language="en" class="${language==='en'?'active':''}">English</button></div></div><div class="alin-desktop-option-row"><span>◐</span><div><b>المظهر</b><small>الوضع النهاري أو الليلي</small></div><div class="alin-desktop-choice-group"><button type="button" data-theme-choice="light" class="${theme==='light'?'active':''}">نهاري</button><button type="button" data-theme-choice="dark" class="${theme==='dark'?'active':''}">ليلي</button></div></div>${optionButton('💬','تواصل معنا','الدعم والاستفسارات','contact')}${optionButton('ⓘ','حول منصة آلين','الخدمات ومعلومات المنصة','about')}</section>`);
    root.querySelector('[data-option-action="account"]')?.addEventListener('click',()=>{close();window.openStudentAuth?.()});
    root.querySelector('[data-option-action="tracking"]')?.addEventListener('click',()=>openTracking());
    root.querySelector('[data-option-action="contact"]')?.addEventListener('click',()=>{close();window.alinContactUs?.()});
    root.querySelector('[data-option-action="about"]')?.addEventListener('click',()=>{close();window.alinAboutPlatform?.()});
    root.querySelectorAll('[data-language]').forEach(button=>button.addEventListener('click',()=>{window.alinSetLanguage?.(button.dataset.language);openOptions()}));
    root.querySelectorAll('[data-theme-choice]').forEach(button=>button.addEventListener('click',()=>{window.alinSetTheme?.(button.dataset.themeChoice);openOptions()}));
  }
  function refreshAccountLabel(){
    const student=window.AlinStudentAuth?.current?.();
    const button=document.querySelector('[data-desktop-control="options"] small');
    if(button)button.textContent=student?.name||'خيارات';
  }
  window.AlinDesktopTools=Object.freeze({openTracking,openOptions,close,refreshAccountLabel});
  window.addEventListener('alin:student-session',refreshAccountLabel);
  document.addEventListener('keydown',event=>{if(event.key==='Escape')close()});
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',refreshAccountLabel,{once:true});else refreshAccountLabel();
})();
