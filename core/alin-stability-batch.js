/* ALIN v4.2.0 — 2026-08-23 stability batch.
   Frontend-only fixes: route persistence, notification read fallback,
   receipt navigation safety, and responsive-role stylesheet loading.
...
(function(){
  ...
  const VERSION=String(window.ALIN_CONFIG?.assetVersion||'4.2.0-stability5-20260823');
  ...
  /* The old receipt guard shipped an accidental trailing comma in a selector.
     Repair only that malformed-selector case while preserving native closest(). */
  ...
  function normalizeRole(value){
    ...
    return role==='delegate'?'courier':role;
  }
  function currentContext(){
    const current=window.current||{};
    return {role:normalizeRole(current.role),id:text(current.id||current.account_id||current.user_id||current.auth_user_id)};
  }
  function routeKey(context=currentContext()){
    return context.role&&context.id?`alin_last_staff_route_v1:${context.role}:${context.id}`:'';
  }
  function readRoute(context=currentContext()){
    ...
    try{
      ...
      return ROLE_TABS[context.role]?.has(tab)?tab:'';
    }catch(_){return ''}
  }
  function saveRoute(role,tab){
    const context=currentContext();
    role=normalizeRole(role||context.role);tab=text(tab);
    if(role!==context.role||!context.id||!ROLE_TABS[role]?.has(tab))return false;
    try{localStorage.setItem(routeKey(context),JSON.stringify({tab,updated_at:new Date().toISOString()}));return true}
  ...
  function clickedRoute(node){
    ...
    const receiptRole=normalizeRole(button.dataset.alin415ReceiptsRole);
    if(receiptRole)return {role:receiptRole,tab:'receipts'};
  ...
  document.addEventListener('click',event=>{
    const route=clickedRoute(event.target);
    if(route)saveRoute(route.role,route.tab);
  },true);

  function pageVisible(id){
    const page=document.getElementById(id);
    return Boolean(page&&!page.classList.contains('hidden')&&!page.hidden);
  }
  function primeRoute(){
    const context=currentContext(),tab=readRoute(context);
    if(!tab)return false;
    if(context.role==='admin'||context.role==='accountant')window.activeAdminTab=tab;
    else if(context.role==='teacher')window.activeTeacherTab=tab;
    return true;
  }
  function restoreRoute(){
    const context=currentContext();
    let tab=readRoute(context);
    if(!tab)return false;
    if(context.role==='accountant'&&!ROLE_TABS.accountant.has(tab))tab='finance';
    try{
      if((context.role==='admin'||context.role==='accountant')&&pageVisible('adminPage')&&typeof window.adminTab==='function'){
        if(window.activeAdminTab!==tab)window.adminTab(tab);
        return true;
      }
      if(context.role==='teacher'&&pageVisible('teacherPage')&&typeof window.teacherTab==='function'){
        if(String(window.TeacherApp?.active||window.activeTeacherTab||'')!==tab)window.teacherTab(tab);
        return true;
      }
      if(context.role==='library'&&pageVisible('libraryPage')){
        const selector=tab==='receipts'?'#libraryReceiptsTab':`.library-v116-tabs [data-library-tab="${CSS.escape(tab)}"]`;
        const button=document.querySelector(selector);
        if(button&&!button.classList.contains('active'))button.click();
        return Boolean(button);
      }
      if(context.role==='courier'&&pageVisible('courierPage')){
        if(tab==='receipts'){
          const button=document.getElementById('courierReceiptsTab');
          if(button&&!button.classList.contains('active'))button.click();
          return Boolean(button);
        }
        const active=document.querySelector('.courier-v161-tabs [data-courier-tab].active')?.dataset.courierTab||'';
        if(active!==tab&&typeof window.renderCourierDashboard==='function')window.renderCourierDashboard(tab,{refresh:false});
        return true;
      }
  ...
  function scheduleRestore(){
    ...
    restoreTimer=setTimeout(restoreRoute,140);
    setTimeout(restoreRoute,420);
  }

  function seenKey(input={}){
    const context=window.AlinNotifications?.context?.(input)||{role:normalizeRole(input.role||window.current?.role||'student'),id:text(input.id||window.current?.id||'guest')};
    return `alin_notifications_ui_seen_v1:${normalizeRole(context.role)||'student'}:${text(context.id)||'guest'}`;
  }
  function seen(input={}){
  ...
  function saveSeen(values,input={}){
    try{localStorage.setItem(seenKey(input),JSON.stringify([...values].slice(-1000)))}catch(_){}
  }
  ...
  function installNotificationFallback(){
    const base=window.AlinNotifications;
    if(!base||base.__alinReadFallbackV1)return false;
    ...
    function isRead(row,input={}){
      if(!row)return true;
      if(seen(input).has(notificationKey(row)))return true;
      return Boolean(originalIsRead(row,input));
    }
    async function markRead(id,input={}){
      const values=seen(input);values.add(String(id));saveSeen(values,input);
      try{return await originalMarkRead(id,input)}
      catch(error){console.warn('[ALIN stability] notification read remote fallback',error);return true}
    }
    async function markAll(input={}){
      const visible=typeof base.visible==='function'?arr(base.visible(input)):[];
      const values=seen(input);visible.forEach((row,index)=>values.add(notificationKey(row,index)));saveSeen(values,input);
      try{return await originalMarkAll(input)}
      catch(error){console.warn('[ALIN stability] notification read-all remote fallback',error);return true}
    }
    function unreadCount(input={}){
      if(typeof base.visible!=='function')return originalUnread(input);
      return arr(base.visible(input)).filter(row=>!isRead(row,input)).length;
    }
  ...
  window.addEventListener('alin:auth-login',scheduleRestore);
  window.addEventListener('alin:auth-restored',scheduleRestore);
  window.addEventListener('alin:page-open',event=>{
    const role=currentContext().role;
    const page=text(event.detail?.page);
    if((role==='admin'||role==='accountant')&&page==='admin')scheduleRestore();
    else if(role===page)scheduleRestore();
  });
  ...
  window.AlinStabilityBatch=Object.freeze({version:VERSION,restoreRoute,saveRoute,installNotificationFallback});
})();
