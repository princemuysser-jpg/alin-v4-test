...
class _AdminBrandingScreenState extends State<AdminBrandingScreen> {
    ...
    try {
      ...
      setState(() {
        ...
        shadow = settings['visual_shadow']?.trim().isNotEmpty == true
            ...
            : 'soft';
        logo.text = settings['platform_logo_path'] ??
            settings['platform_logo_url'] ??
            '';
        logoDark.text = settings['platform_logo_dark_path'] ?? '';
        icon.text = settings['platform_icon_path'] ??
  ...
  bool _validHex(String value) => RegExp(r'^#[0-9a-fA-F]{6}$').hasMatch(value.trim());
  ...
  Future<String?> _pickAndUpload(String kind) async {
    ...
    final path = 'brand/$kind/${DateTime.now().millisecondsSinceEpoch}.$extension';
  ...
  Future<void> save() async {
    ...
    try {
      ...
      BusinessBrand.configure(values.map((key, value) => MapEntry(key, '$value')));
      if (!mounted) return;
  ...
  void applyPreset(String value) {
    final preset = switch (value) {
      'sky-calm' => ['#315D7A', '#9EB9C9', '#F3F7F9', '#FFFFFF', '#4E806D', '#B38A4E', '#A95B5B', 'Tajawal', '20'],
      'sage-calm' => ['#456B5F', '#B9A878', '#F4F7F3', '#FFFFFF', '#3F7B5D', '#A98045', '#A95555', 'Cairo', '20'],
      'sand-calm' => ['#6F5948', '#C2A278', '#FAF7F1', '#FFFDFA', '#5F8067', '#AD7B3C', '#AA5952', 'Tajawal', '18'],
      'lavender-calm' => ['#5D5D7D', '#B8AFCA', '#F7F6FA', '#FFFFFF', '#5E806E', '#A98652', '#A95B67', 'Cairo', '22'],
      'rose-calm' => ['#765861', '#D0AEB5', '#FAF6F7', '#FFFFFF', '#5B7D6B', '#AA8050', '#A64F5A', 'Tajawal', '22'],
      'graphite-calm' => ['#3F4B59', '#AEB7C0', '#F4F6F8', '#FFFFFF', '#4D7B69', '#9E7D4E', '#A65353', 'Cairo', '16'],
      _ => ['#0B3158', '#C9A24A', '#F6F8FB', '#FFFFFF', '#2F7D62', '#B98532', '#B44B4B', 'Cairo', '18'],
    };
    setState(() {
      theme = value;
        ...
        children: [
          ...
          _section(
            ...
            child: Column(children: [
              ...
              _imageField('الشعار للوضع الداكن', logoDark, () => uploadLogo(dark: true)),
              const SizedBox(height: 10),
              _imageField('أيقونة المنصة', icon, uploadIcon),
              const SizedBox(height: 8),
              const Text('PNG / JPG / WebP — الحد الأقصى 3MB. تحفظ الصور في Supabase / alin-files.', style: TextStyle(color: BusinessBrand.muted, fontSize: 12)),
            ]),
          ),
          const SizedBox(height: 12),
          _section(
              ...
              builder: (context, c) {
                ...
                if (c.maxWidth >= 650) {
                  return Wrap(
                    spacing: 10,
                    runSpacing: 10,
                    children: fields.map((e) => SizedBox(width: (c.maxWidth - 10) / 2, child: e)).toList(),
                  );
                }
                return Column(children: fields.map((e) => Padding(padding: const EdgeInsets.only(bottom: 9), child: e)).toList());
              },
            ),
          ),
          const SizedBox(height: 12),
          _section(
            ...
            child: Column(children: [
              DropdownButtonFormField<String>(
                ...
                items: const ['Cairo', 'Tajawal', 'Arial'].map((value) => DropdownMenuItem(value: value, child: Text(value))).toList(),
              ...
              const SizedBox(height: 14),
              Row(children: [
                const Text('استدارة البطاقات', style: TextStyle(fontWeight: FontWeight.w800)),
                const Spacer(),
    ...
    return Card(
      child: Padding(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
          const Text('معاينة الهوية', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w900)),
          const SizedBox(height: 12),
          Container(
            ...
            child: Column(children: [
              Container(
                ...
                decoration: BoxDecoration(color: p, borderRadius: BorderRadius.circular(radius)),
                child: Row(children: [
                  _previewImage(icon.text, 42, fallback: Icons.auto_awesome_mosaic_rounded),
                  const SizedBox(width: 10),
                  const Expanded(child: Text('منصة آلين', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w900, fontSize: 18))),
                ]),
              ),
              const SizedBox(height: 10),
              Container(
                padding: const EdgeInsets.all(16),
                decoration: BoxDecoration(color: surface, borderRadius: BorderRadius.circular(radius)),
                child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  Row(children: [_previewImage(logo.text, 58, fallback: Icons.school_rounded), const SizedBox(width: 10), const Expanded(child: Text('لوحة آلين للأعمال', style: TextStyle(fontWeight: FontWeight.w900)))]),
                  const SizedBox(height: 14),
                  Row(children: [Expanded(child: Container(height: 10, decoration: BoxDecoration(color: p, borderRadius: BorderRadius.circular(9)))), const SizedBox(width: 8), Expanded(child: Container(height: 10, decoration: BoxDecoration(color: s, borderRadius: BorderRadius.circular(9))))]),
                ]),
              ),
            ]),
          ),
  ...
  Widget _previewImage(String url, double size, {required IconData fallback}) {
    if (url.trim().isEmpty) {
      return Container(width: size, height: size, decoration: BoxDecoration(color: Colors.white.withValues(alpha: .16), borderRadius: BorderRadius.circular(size * .24)), child: Icon(fallback, color: Colors.white));
    }
    return ClipRRect(
      borderRadius: BorderRadius.circular(size * .24),
      child: Image.network(url.trim(), width: size, height: size, fit: BoxFit.contain, errorBuilder: (_, __, ___) => Container(width: size, height: size, color: Colors.white12, child: Icon(fallback, color: Colors.white))),
    );
  }
  ...
  Widget _section({required String title, required IconData icon, required Widget child}) => Card(
        child: Padding(
          padding: const EdgeInsets.all(16),
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(children: [Icon(icon, color: BusinessBrand.navy), const SizedBox(width: 8), Text(title, style: const TextStyle(fontSize: 17, fontWeight: FontWeight.w900))]),
            const SizedBox(height: 14),
            child,
          ]),
        ),
  ...
  Widget _imageField(String label, TextEditingController controller, Future<void> Function() upload) => Row(
        children: [
          Expanded(child: TextField(controller: controller, onChanged: (_) => setState(() {}), decoration: InputDecoration(labelText: label, hintText: 'رابط الصورة أو ارفع صورة'))),
          const SizedBox(width: 8),
          FilledButton.icon(onPressed: uploadingLogo || uploadingIcon ? null : upload, icon: const Icon(Icons.upload_rounded), label: const Text('رفع')),
        ],
      );

  Widget _colorField(String label, TextEditingController controller) => TextField(
        ...
        decoration: InputDecoration(
          ...
          prefixIcon: Padding(
            padding: const EdgeInsets.all(12),
            child: DecoratedBox(decoration: BoxDecoration(color: _color(controller.text, Colors.grey), borderRadius: BorderRadius.circular(7))),
          ),
        ),
      );
}
