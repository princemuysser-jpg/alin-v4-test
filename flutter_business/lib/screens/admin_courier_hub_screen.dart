...
class _AdminCourierHubScreenState extends State<AdminCourierHubScreen> {
  ...
  Future<void> load() async {
    ...
    try {
      ...
      setState(() {
        couriers = accounts
            .where((e) => const {'courier', 'delegate'}.contains('${e['role']}') && e['deleted_at'] == null)
            .toList();
        orders = values[1];
      });
    } catch (e) {
  ...
  bool isClosed(Map<String, dynamic> order) =>
      const {'completed', 'delivered', 'done', 'cancelled', 'canceled', 'rejected'}
  ...
  List<Map<String, dynamic>> get filtered {
    ...
    return couriers.where((row) {
      final text = '${row['name'] ?? ''} ${row['username'] ?? ''} ${row['phone'] ?? ''} ${row['area'] ?? ''}'.toLowerCase();
  ...
  Widget build(BuildContext context) {
    ...
    final completedOrders = orders.where((e) {
      final courier = '${e['courier_id'] ?? e['delegate_id'] ?? ''}';
      return courier.isNotEmpty && isClosed(e) && '${e['status']}'.toLowerCase() != 'cancelled';
    }).length;
              ...
              : RefreshIndicator(
                  ...
                  child: ListView(
                    ...
                    children: [
                      ...
                      LayoutBuilder(
                        builder: (context, c) {
                          ...
                          return GridView.count(
                            ...
                            children: [
                              _metric('كل المندوبين', '${couriers.length}', Icons.groups_rounded),
                              _metric('الفعالون', '$active', Icons.check_circle_rounded),
                              _metric('طلبات حالية', '$currentOrders', Icons.local_shipping_rounded),
                              _metric('طلبات مكتملة', '$completedOrders', Icons.task_alt_rounded),
                            ],
                          );
                        },
                      ),
                      ...
                      const SizedBox(height: 12),
                      if (filtered.isEmpty)
                        const Card(child: Padding(padding: EdgeInsets.all(28), child: Center(child: Text('لا يوجد مندوبون'))))
                      else
        ...
        child: LayoutBuilder(
          builder: (context, c) {
            final title = const Column(
              ...
              children: [
                Text('إدارة المندوبين والتوصيل', style: TextStyle(color: Colors.white, fontSize: 21, fontWeight: FontWeight.w900)),
                SizedBox(height: 4),
                Text('المندوبون والطلبات والمناطق والحسابات في مركز واحد.', style: TextStyle(color: Colors.white70)),
              ],
            );
            final actions = Wrap(
              spacing: 8,
              ...
              children: [
                FilledButton.icon(
                  ...
                  onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (_) => AdminDeliveryPricingScreen(repository: widget.repository))),
                  icon: const Icon(Icons.map_rounded),
                  label: const Text('المناطق والأسعار'),
                ),
                OutlinedButton.icon(
                  style: OutlinedButton.styleFrom(foregroundColor: Colors.white, side: const BorderSide(color: Colors.white54)),
                  onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (_) => AdminFinanceV2Screen(repository: widget.repository))),
                  icon: const Icon(Icons.account_balance_wallet_rounded),
                  label: const Text('الحسابات'),
                ),
              ],
            );
            if (c.maxWidth >= 750) return Row(children: [Expanded(child: title), actions]);
            return Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [title, const SizedBox(height: 14), actions]);
          },
        ),
      );
  ...
  Widget _courierCard(Map<String, dynamic> courier) {
    ...
    final availability = '${courier['availability'] ?? courier['courier_status'] ?? courier['status'] ?? ''}';
    return Card(
      ...
      child: Padding(
        ...
        child: LayoutBuilder(
          builder: (context, c) {
            final info = Row(
              children: [
                CircleAvatar(
                  ...
                  child: Text(
                    '${courier['name'] ?? 'م'}'.isEmpty ? 'م' : '${courier['name'] ?? 'م'}'.substring(0, 1),
                    style: const TextStyle(fontWeight: FontWeight.w900, color: BusinessBrand.navy),
                  ),
                ),
                const SizedBox(width: 12),
                Expanded(
                  child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    Text('${courier['name'] ?? courier['username'] ?? 'مندوب'}', style: const TextStyle(fontSize: 17, fontWeight: FontWeight.w900)),
                    const SizedBox(height: 3),
                    Text('${courier['phone'] ?? 'بدون هاتف'} • ${courier['area'] ?? 'بدون منطقة'}'),
            ...
            return Row(children: [Expanded(child: info), Text('إجمالي الطلبات ${assigned.length}', style: const TextStyle(fontWeight: FontWeight.w800, color: BusinessBrand.navy))]);
          },
        ),
      ),
    );
