...
class _TeacherDashboardScreenState extends State<TeacherDashboardScreen> {
    ...
    try {
      ...
      final results = await Future.wait([
        client
            .from('booklets')
            .select(
              'id,title,subject,grade,term,edition,year,price,status,publish_status,published,is_published,teacher_approved,admin_note,created_at,updated_at',
            )
        ...
        client
            ...
            .select(
              'id,order_number,title,kind,qty,unit_price,total,status,teacher_profit,created_at,updated_at,teacher_id',
            )
            .eq('teacher_id', widget.account.id)
            .order('created_at', ascending: false)
            .limit(300),
        client
            ...
            .select(
              'id,receipt_number,teacher_id,amount,payment_method,status,note,created_at,updated_at',
            )
            .eq('teacher_id', widget.account.id)
            .order('created_at', ascending: false)
            .limit(200),
  ...
  Widget build(BuildContext context) {
    ...
    final earned = doneOrders.fold<num>(
      0,
      (sum, o) => sum + number(o['teacher_profit']),
    );
          ...
          .map(
            (o) => Card(
              ...
              child: ListTile(
                ...
                subtitle: Text(
                  'النسخ: ${o['qty'] ?? 1} • ${_date(o['created_at'])} • الحالة: ${o['status'] ?? '-'}',
                ),
                trailing: Column(
                  mainAxisAlignment: MainAxisAlignment.center,
                  crossAxisAlignment: CrossAxisAlignment.end,
          ...
          ...settlements.map(
            (s) => Card(
              ...
              child: ListTile(
                ...
                subtitle: Text(
                  '${_date(s['created_at'])}${'${s['payment_method'] ?? ''}'.isNotEmpty ? ' — ${s['payment_method']}' : ''}',
                ),
                trailing: Text(
                  money(s['amount']),
                  style: const TextStyle(
