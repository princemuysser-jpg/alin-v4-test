...
class _AdminReportsScreenState extends State<AdminReportsScreen> {
  ...
  DateTime? orderDate(Map<String, dynamic> row) => DateTime.tryParse(
  ...
  bool isBooklet(Map<String, dynamic> row) {
    ...
    return value.contains('book') ||
        value.contains('booklet') ||
        value.contains('mulazama');
  }
  ...
  List<Map<String, dynamic>> get filteredOrders {
    ...
    return orders.where((row) {
      ...
      final text =
          '${row['order_number'] ?? ''} ${row['student_name'] ?? ''} ${row['title'] ?? ''}'
              .toLowerCase();
  ...
  Set<String> orderKeys(Map<String, dynamic> row) =>
      {'${row['id'] ?? ''}', '${row['order_number'] ?? ''}'}..remove('');

  Set<String> ledgerKeys(Map<String, dynamic> row) =>
      {'${row['order_id'] ?? ''}', '${row['order_number'] ?? ''}'}..remove('');

  List<Map<String, dynamic>> get filteredLedger {
    if (period == 'all' && kind == 'all' && search.trim().isEmpty)
      return ledger;
    final allowed = filteredOrders.expand(orderKeys).toSet();
    return ledger
        .where((row) => ledgerKeys(row).any(allowed.contains))
        .toList();
  }
  ...
  Map<String, Map<String, dynamic>> rankBy(
    ...
    for (final row in rows) {
      ...
      final current = result.putIfAbsent(
        key,
        () => {'label': labelOf(row), 'qty': 0, 'total': 0},
      );
      current['qty'] =
          n(current['qty']) + (n(row['qty']) <= 0 ? 1 : n(row['qty']));
  ...
  Widget build(BuildContext context) {
    ...
    final platform = moneyRows.fold<num>(
      0,
      (sum, row) => sum + n(row['admin'] ?? row['alin']),
    );
    final teachers = moneyRows.fold<num>(
      0,
      (sum, row) => sum + n(row['teacher']),
    );
    final libraries = moneyRows.fold<num>(
      0,
      (sum, row) => sum + n(row['library']),
    );
    final couriers = moneyRows.fold<num>(
      0,
      (sum, row) => sum + n(row['courier'] ?? row['delegate']),
    );
    ...
    final topItems = sortedRank(
      rankBy(
        rows,
        (row) => '${row['item_id'] ?? row['title'] ?? ''}',
        (row) => '${row['title'] ?? 'عنصر'}',
      ),
    );
    final topLibraries = sortedRank(
      rankBy(
        rows,
          ...
          : RefreshIndicator(
              ...
              child: ListView(
                ...
                children: [
                  ...
                  LayoutBuilder(
                    builder: (context, c) =>
                        c.maxWidth >= 850 ? _table(rows) : _cards(rows),
                  ),
                ],
              ),
            ),
            ...
            final fields = [
              ...
              TextField(
                ...
                decoration: const InputDecoration(
                  ...
                  hintText: 'رقم الطلب أو الطالب أو العنصر',
                ),
              ),
            ];
            if (wide) {
              ...
              return Padding(
                ...
                child: Row(
                  children: [
                    ...
                    Text(
                      '${n(row['qty']).round()} • ${money(row['total'])}',
    ...
    if (rows.isEmpty)
      return const Card(
        child: Padding(
          ...
          child: Center(child: Text('لا توجد طلبات ضمن الفلتر')),
        ),
      );
    return Card(
      ...
      children: rows
          ...
          .map(
            (row) => Card(
              ...
              child: ListTile(
                title: Text(
                  '${row['order_number'] ?? row['id'] ?? '—'} — ${row['title'] ?? 'طلب'}',
                  style: const TextStyle(fontWeight: FontWeight.w900),
                ),
                subtitle: Text(
                  '${row['student_name'] ?? '—'} • ${_date(row['created_at'])} • ${row['status'] ?? '—'}',
                ),
                trailing: Text(
                  money(row['total']),
                  style: const TextStyle(fontWeight: FontWeight.w900),
