...
class _CourierProfileScreenState extends State<CourierProfileScreen> {
  ...
  Future<void> saveAvailability() async {
    ...
    try {
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text('تم حفظ حالة العمل')));
    } catch (e) {
      if (!mounted) return;
  ...
  String get phone =>
      '${profile['phone'] ?? widget.account.phone ?? ''}'.trim();
        ...
        children: [
          ...
          SegmentedButton<String>(
            ...
            onSelectionChanged: saving
                ? null
                : (value) => setState(() => availability = value.first),
          ),
          const SizedBox(height: 14),
          FilledButton.icon(
