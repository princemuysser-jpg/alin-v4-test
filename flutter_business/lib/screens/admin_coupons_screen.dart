...
class _AdminCouponsScreenState extends State<AdminCouponsScreen> {
  ...
  int used(Map<String, dynamic> row) => n(row['used_count'] ?? row['usage_count']).round();
  int limit(Map<String, dynamic> row) => n(row['max_uses'] ?? row['usage_limit']).round();
  ...
  List<Map<String, dynamic>> get visible {
    ...
    return coupons.where((row) {
      final haystack = '${row['code'] ?? ''} ${row['applies_to'] ?? ''} ${row['status'] ?? ''}'.toLowerCase();
  ...
  Future<void> edit([Map<String, dynamic>? row]) async {
    ...
    final value = TextEditingController(text: row == null ? '' : '${n(row['discount_value']).round()}');
    final maxUses = TextEditingController(text: row == null ? '0' : '${limit(row)}');
    String type = '${row?['discount_type'] ?? 'percent'}';
    String applies = '${row?['applies_to'] ?? 'all'}';
    String status = '${row?['status'] ?? 'active'}';
    DateTime? expiry = DateTime.tryParse('${row?['expires_at'] ?? ''}')?.toLocal();
              ...
              child: Column(
                ...
                children: [
                  TextField(
                    ...
                    decoration: const InputDecoration(labelText: 'كود الخصم', hintText: 'ALIN20'),
                  ),
                  ...
                  TextField(
                    controller: maxUses,
                    keyboardType: TextInputType.number,
                    decoration: const InputDecoration(labelText: 'عدد الاستخدامات', helperText: '0 = بدون حد'),
                  ),
                  const SizedBox(height: 12),
                  DropdownButtonFormField<String>(
                    ...
                    onChanged: (v) => setDialogState(() => applies = v ?? 'all'),
                  ),
                  const SizedBox(height: 12),
                  DropdownButtonFormField<String>(
                    value: status,
                  ...
                  ListTile(
                    ...
                    subtitle: Text(expiry == null ? 'غير محدد' : '${expiry!.year}/${expiry!.month.toString().padLeft(2, '0')}/${expiry!.day.toString().padLeft(2, '0')}'),
                    trailing: Wrap(
                      spacing: 4,
                      children: [
                        if (expiry != null)
                        ...
                        IconButton(
                          ...
                          onPressed: () async {
                            ...
                            if (selected != null) setDialogState(() => expiry = selected);
                          },
            ...
            FilledButton(
              onPressed: () {
                ...
                if (normalized.length < 2 || discount <= 0) {
                  ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('أكمل كود وقيمة الخصم')));
                  return;
                }
                Navigator.pop(dialogContext, {
                  ...
                  'expires_at': expiry == null
                      ? null
                      : DateTime(expiry!.year, expiry!.month, expiry!.day, 23, 59, 59).toUtc().toIso8601String(),
                });
              },
    ...
    setState(() => busy = true);
    try {
      final duplicate = coupons.any((coupon) =>
          '${coupon['code'] ?? ''}'.trim().toUpperCase() == '${payload['code']}' &&
          '${coupon['id']}' != '${row?['id']}');
      ...
      if (row == null) {
        ...
        await widget.repository.client.from('coupons').update(payload).eq('id', row['id']);
      }
      await load();
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('تم حفظ الكوبون')));
    } catch (e) {
      if (!mounted) return;
  ...
  Future<void> toggle(Map<String, dynamic> row) async {
    ...
    setState(() => busy = true);
    try {
      final next = '${row['status'] ?? 'active'}' == 'active' ? 'disabled' : 'active';
      await widget.repository.client.from('coupons').update({'status': next}).eq('id', row['id']);
      await load();
    } catch (e) {
  ...
  Future<void> remove(Map<String, dynamic> row) async {
    ...
    setState(() => busy = true);
    try {
      await widget.repository.client.from('coupons').delete().eq('id', row['id']);
      await load();
    } catch (e) {
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('$e'.replaceFirst('Exception: ', ''))));
    } finally {
              ...
              : RefreshIndicator(
                  ...
                  child: ListView(
                    ...
                    children: [
                      LayoutBuilder(
                        builder: (context, c) {
                          ...
                          return GridView.count(
                            ...
                            children: [
                              _metric('إجمالي الكوبونات', '${coupons.length}', Icons.confirmation_number_rounded),
                              _metric('نشطة', '$activeCount', Icons.check_circle_rounded),
                              _metric('مرات الاستخدام', '$usedCount', Icons.shopping_cart_checkout_rounded),
                              _metric('منتهية', '$expired', Icons.event_busy_rounded),
                            ],
                          );
                        },
                      ),
                      ...
                      const SizedBox(height: 14),
                      if (visible.isEmpty)
                        const Card(child: Padding(padding: EdgeInsets.all(24), child: Center(child: Text('لا توجد كوبونات'))))
                      else
          ...
          children: [
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  Text(value, style: const TextStyle(fontSize: 19, fontWeight: FontWeight.w900, color: BusinessBrand.navy)),
        ...
        child: Column(
          ...
          children: [
            Row(
              children: [
                Expanded(
                  child: Text('${row['code'] ?? '—'}', style: const TextStyle(fontSize: 18, fontWeight: FontWeight.w900, color: BusinessBrand.navy)),
                ),
                Chip(
                  avatar: Icon(isActive ? Icons.check_circle_rounded : Icons.pause_circle_rounded, size: 17),
                  label: Text(isActive ? 'نشط' : 'متوقف/منتهي'),
                ),
              ],
            ),
            const SizedBox(height: 6),
            Text('$discount خصم • ${_appliesLabel('${row['applies_to'] ?? 'all'}')}'),
            const SizedBox(height: 5),
            Text('الاستخدام ${used(row)}/${max == 0 ? '∞' : max} • الانتهاء ${expiry == null ? 'غير محدد' : '${expiry.year}/${expiry.month}/${expiry.day}'}'),
            const SizedBox(height: 10),
            Wrap(
              spacing: 8,
              runSpacing: 8,
              children: [
                OutlinedButton.icon(onPressed: busy ? null : () => edit(row), icon: const Icon(Icons.edit_rounded), label: const Text('تعديل')),
                OutlinedButton.icon(
                  onPressed: busy ? null : () => toggle(row),
                  icon: Icon('${row['status'] ?? 'active'}' == 'active' ? Icons.pause_rounded : Icons.play_arrow_rounded),
                  label: Text('${row['status'] ?? 'active'}' == 'active' ? 'إيقاف' : 'تشغيل'),
                ),
                TextButton.icon(onPressed: busy ? null : () => remove(row), icon: const Icon(Icons.delete_outline_rounded), label: const Text('حذف')),
              ],
            ),
          ],
        ),
