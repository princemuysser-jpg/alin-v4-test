...
class AdminDesktopDashboardScreen extends StatefulWidget {
  ...
  @override
  State<AdminDesktopDashboardScreen> createState() =>
      _AdminDesktopDashboardScreenState();
}

class _AdminDesktopDashboardScreenState
    extends State<AdminDesktopDashboardScreen> {
  int section = 0;
  bool loading = true;
  String? error;
  ...
  bool _closed(Map<String, dynamic> o) => const [
    'completed',
    'delivered',
    'cancelled',
    'rejected',
  ...
  List<Map<String, dynamic>> get visibleOrders {
    ...
    if (orderFilter == 'done')
      return grouped.where(receiptGroupCompleted).toList();
    if (orderFilter == 'cancelled')
      return grouped
          .where(
            (e) => ((e['_items'] as List).cast<Map<String, dynamic>>()).every(
              (row) =>
                  const ['cancelled', 'rejected'].contains('${row['status']}'),
            ),
          )
          .toList();
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(
            widget.account.role == 'accountant' ? 'حسابات' : 'مدير',
  ...
  Widget _homePage() {
    final cards = [
      ...
      (
        'ربح المنصة',
        _money(finance['platform_profit'] ?? summary['platform_profit']),
          ...
          children: [
            Expanded(
              child: Column(
                ...
                children: [
                  Text(
                    'إدارة أعمال آلين من شاشة واحدة',
                    style: TextStyle(
                      color: Colors.white,
                      fontSize: 24,
                      fontWeight: FontWeight.w900,
                  ...
                  SizedBox(height: 6),
                  Text(
                    'متابعة الطلبات والحسابات والمالية والمحتوى بصورة أوضح على الكمبيوتر.',
                  ...
                  cells: [
                    ...
                    DataCell(
                      PopupMenuButton<String>(
                        ...
                        itemBuilder: (_) => [
                          if (_delivery(o) && !_closed(o))
                            const PopupMenuItem(
                              value: 'courier',
                              child: Text('تعيين مندوب'),
                            ),
                          if (!_closed(o))
                            const PopupMenuItem(
                              value: 'status',
    ...
    Row(
      children: [
        ...
        DropdownButton<String>(
          ...
          onChanged: (v) =>
              v == null ? null : setState(() => accountFilter = v),
        ),
      ],
    ),
    const SizedBox(height: 14),
    ...
    LayoutBuilder(
      builder: (context, c) {
        final cards = [
          ...
          (
            'مستحق المدرسين',
            finance['teacher_remaining'],
            Icons.school_rounded,
          ),
          (
            'ذمم التحصيل',
            finance['collector_remaining'],
            Icons.account_balance_wallet_rounded,
          ),
          (
            'مستحق توريد الكتب',
            finance['book_supplier_pending'],
        ...
        return GridView.count(
          ...
          children: cards
              .map((e) => _metric(e.$1, _money(e.$2), e.$3))
              .toList(),
        );
      },
    ),
            ...
            const Expanded(
              child: Column(
                ...
                children: [
                  ...
                  SizedBox(height: 4),
                  Text(
                    'التسويات، الأرصدة، عكس السندات، وحسابات الموردين من المصدر الرسمي.',
                  ),
                ],
              ),
            ),
      ...
      builder: (context, c) {
        return GridView.count(
          ...
          children: [
            _metric(
              'الملازم',
              '${booklets.where((e) => e['deleted_at'] == null).length}',
              Icons.menu_book_rounded,
            ),
            _metric(
              'المنتجات',
              '${products.where((e) => e['deleted_at'] == null).length}',
        ...
        children: booklets
            ...
            .map(
              (b) => ListTile(
                ...
                trailing: Chip(
                  label: Text(
                    '${b['published'] == true || b['is_published'] == true ? 'منشورة' : 'غير منشورة'}',
                  ),
                ),
              ),
            )
  ...
  Future<void> _assignCourier(Map<String, dynamic> order) async {
    ...
    String? selected = '${order['courier_id'] ?? order['delegate_id'] ?? ''}'
        .trim();
          ...
          builder: (context, setLocal) => DropdownButtonFormField<String>(
            ...
            items: active
                .map(
                  (c) => DropdownMenuItem(
                    value: '${c['id']}',
                    child: Text('${c['name'] ?? c['username'] ?? c['id']}'),
                  ),
                )
                .toList(),
  ...
  Future<void> _changeStatus(Map<String, dynamic> order) async {
    ...
    String selected = statuses.contains('${order['status']}')
        ? '${order['status']}'
        : 'processing';
    final result = await showDialog<String>(
      context: context,
      builder: (context) => AlertDialog(
        ...
        content: StatefulBuilder(
          builder: (context, setLocal) => DropdownButtonFormField<String>(
            initialValue: selected,
            items: statuses
                .map((s) => DropdownMenuItem(value: s, child: Text(_status(s))))
                .toList(),
            onChanged: (v) {
    ...
    try {
      await widget.repository.adminTransitionOrder(
        '${order['id']}',
        result,
        reason: reason,
      );
