...
assert(migration.includes('create table if not exists public.student_notification_reads'),'student read state must use its own table');
assert(migration.includes('references public.student_profiles(id)'),'student read table must be bound to student profiles');
assert(migration.includes('alter table public.student_notification_reads enable row level security'),'student read table must have RLS enabled');
assert(migration.includes('revoke all on table public.student_notification_reads from anon,authenticated'),'student read table must not be directly exposed');
assert(!migration.includes('public.notification_reads'),'student RPCs must not write staff notification_reads');
